...
class BillAmountScreen extends Component {
  constructor(props){
    super(props);
    this.state = {
      number  : '',
  ...
  componentDidMount(){
    ...
    this.setState({
      id               : state.params.id,
      billNumber       : state.params.billNumber,
      referenceNumber  : state.params.referenceNumber,
    })
  }
  ...
  nextScreen = () => {
    if(this.state.number.length > 0){
      this.props.navigation.navigate('BillPinScreen', {billNumber : this.state.billNumber,referenceNumber: this.state.referenceNumber, amount : this.state.number, id : this.state.id});
    }
  }
    ...
    return (
      <View style={styles.container}>
        ...
        <View style={styles.section12}>
          ...
          <LinearGradient colors={['#7A00D2', '#5217ee']} style={{marginHorizontal: 20, elevation: 5, borderRadius: 10, paddingHorizontal: 15,paddingVertical: 15, alignItems: 'center'}}>
            <View style={{flex: 1,marginLeft: 15, justifyContent: 'center'}}>
              <Text style={{color: 'white', fontSize: 16}}>Bill Number : {this.state.billNumber}</Text>
              <Text style={{color: 'white', fontSize: 16}}>Reference Number : {this.state.referenceNumber}</Text>
            </View>
          </LinearGradient>
