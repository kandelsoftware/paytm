...
import {AC_BillPayProcess} from '../../../reducers/Money_Reducer/actions';

class BillPinScreen extends Component {
  constructor(props){
    super(props);
    this.state = {
      number1 : '',
  ...
  nextScreen = () => {
    ...
    formData.billNo    = this.state.billNumber;
    formData.referenceNo   = this.state.referenceNumber;
    formData.amount      = this.state.amount;
    formData.pin         = number1+number2+number3+number4;
    formData.companyId   = this.state.id;
    this.props.BillPayProcess(formData);
  }

  componentWillReceiveProps(nextProps){
    if(nextProps.MoneyState.billPaySuccess == true){
      this.props.navigation.navigate('BillSuccessScreen');
    }
  }
...
function mapDispatchToProps(dispatch){
  return bindActionCreators({
    BillPayProcess : AC_BillPayProcess,
  }, dispatch)
}
 
export default connect(mapStateToProps,mapDispatchToProps)(BillPinScreen);
