...
class UserRequestAmountScreen extends Component {
  constructor(props){
    super(props);
    this.state = {
      number  : '',
  ...
  nextScreen = () => {
    if(this.state.number.length > 0){
      ...
      formData.mobileNumber = this.props.MoneyState.userDetails.mobileNumber;
      formData.amount       = this.state.number;
      this.props.RequestAmount(formData);
    }
  }

  componentWillReceiveProps(nextProps){
    if(nextProps.MoneyState.requestSuccess == true){
      const resetAction = StackActions.reset({
        index: 0,
        actions: [NavigationActions.navigate({ routeName: 'RequestSuccess1Screen'})],
      });
  ...
  render() {
    ...
    if(this.props.MoneyState.userDetails.imageUrl != '' && this.props.MoneyState.userDetails.imageUrl != null && this.props.MoneyState.userDetails.imageUrl != undefined){
      imageUrl  = {uri : 'data:image/png;base64,'+this.props.MoneyState.userDetails.imageUrl};
    }
    return (
        ...
        <View style={styles.section12}>
          ...
          <LinearGradient colors={['#7A00D2', '#5217ee']} style={{marginHorizontal: 20, elevation: 5, borderRadius: 10, paddingHorizontal: 15,paddingVertical: 10,flexDirection: 'row'}}>
            ...
            <Text style={{color: 'white', fontSize: 16}}>
              {this.props.MoneyState.userDetails.fullName}</Text>
              <Text style={{color: 'white', fontSize: 16}}>
              {this.props.MoneyState.userDetails.mobileNumber}</Text>
            </View>
          </LinearGradient>
...
export default connect(mapStateToProps, mapDispatchToProps)(UserRequestAmountScreen);
