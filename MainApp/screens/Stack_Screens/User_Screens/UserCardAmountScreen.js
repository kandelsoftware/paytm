...
import {AC_StripeProcess} from '../../../reducers/Money_Reducer/actions';
import {AC_GetUserBalance } from '../../../reducers/Profile_Reducer/actions';

class UserCardAmountScreen extends Component {
  constructor(props){
    super(props);
    this.state = {
      number  : '',
  ...
  nextScreen = () => {
    if(this.state.number.length > 0){
      ...
      this.props.StripeProcess(formData)
    }
  }

  componentWillReceiveProps(nextProps){
    if(nextProps.MoneyState.stripeSuccess == true){
      const resetAction = StackActions.reset({
        index: 0,
        actions: [NavigationActions.navigate({ routeName: 'CardSuccessScreen' })],
      });
  ...
  render() {
    return (
      <View style={styles.container}>
        ...
        <View style={{marginTop: Platform.OS === 'ios' ? 35 : 35,}}>
...
function mapDispatchToProps(dispatch){
  return bindActionCreators({
    StripeProcess  : AC_StripeProcess,
    GetUserBalance : AC_GetUserBalance
  }, dispatch);
}

export default connect(mapStateToProps, mapDispatchToProps)(UserCardAmountScreen);
