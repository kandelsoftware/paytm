...
import { AC_UserScanToPay, AC_UserPaying } from '../../../reducers/Money_Reducer/actions';

class UserPayPinScreen extends Component {
  constructor(props){
    super(props);
    this.state = {
      number1 : '',
  ...
  nextScreen = () => {
    ...
    formData.userId       = this.props.MoneyState.payeeDetails.id;
    formData.pin          = number1+number2+number3+number4;
    this.props.UserPaying(formData);
  }

  componentWillReceiveProps(nextProps){
    if(nextProps.MoneyState.paymentSuccess == true){
      const resetAction = StackActions.reset({
        index: 0,
        actions: [NavigationActions.navigate({ routeName: 'UserPaymentSuccessScreen' })],
      });
...
function mapDispatchToProps(dispatch){
  return bindActionCreators({UserScanToPay : AC_UserScanToPay,
                             UserPaying    : AC_UserPaying}, dispatch);
}

export default connect(mapStateToProps, mapDispatchToProps)(UserPayPinScreen);
