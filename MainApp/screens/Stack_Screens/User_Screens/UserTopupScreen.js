...
class UserTopupScreen extends Component {
  constructor(props){
    super(props);
    this.state = {
      user : false,
  ...
  nextScreen = () => {
    ...
    if(phoneNoValid) {
      ...
      if(phoneNoValid == true){
        ...
        // this.props.navigation.navigate('TopupAmountScreen', {mobileNumber : mobileNumber, companyId: this.state.id});
      }else {
      ...
      let formData = {};
      formData.AccessToken  = this.props.AccountState.token;
      formData.mobileNumber = this.props.ProfileState.userDetails.mobileNumber;
      this.setState({
        mobileNumber : this.props.ProfileState.userDetails.mobileNumber
      })
      this.props.CheckUserNumber(formData);
    }
  }
  ...
  componentWillReceiveProps(nextProps){
    if(this.props.MoneyState.numberValid == false && nextProps.MoneyState.numberValid == true){
      this.props.ResetNumber();
       this.props.navigation.navigate('TopupAmountScreen', {mobileNumber : this.state.mobileNumber, companyId: this.state.id});
    }
  }
...
export default connect(mapStateToProps, mapDispatchToProps)(UserTopupScreen);
