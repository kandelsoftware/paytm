...
class WithdrawAmountScreen extends Component {
  constructor(props){
    super(props);
    this.state = {
      number  : '',
  ...
  componentDidMount(){
    ...
    this.setState({
      id : state.params.id,
      acNumber : state.params.acNumber
    })
  }
  ...
  nextScreen = () => {
    if(this.state.number.length > 0){
      if(this.props.ProfileState.userBalance.balance > this.state.number){
        this.props.navigation.navigate('WithdrawPinScreen', {id: this.state.id, acNumber: this.state.acNumber, amount: this.state.number});
      }else {
        alert('Insufficient balance, enter lesser amount');
      }
    }
  }
  render() {
        ...
        <View style={styles.section12}>
          ...
          <LinearGradient colors={['#7A00D2', '#5217ee']} style={{marginHorizontal: 20, elevation: 5, borderRadius: 10, paddingHorizontal: 15,paddingVertical: 15, alignItems: 'center'}}>
            <View style={{flex: 1,marginLeft: 15, justifyContent: 'center'}}>
              <Text style={{color: 'white', fontSize: 16}}>Account Number : {this.state.acNumber}</Text>
            </View>
          </LinearGradient>
...
export default connect(mapStateToProps)(WithdrawAmountScreen);
