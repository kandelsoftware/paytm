...
import {AC_WithdrawAmount} from '../../../reducers/Money_Reducer/actions';

class WithdrawPinScreen extends Component {
  constructor(props){
    super(props);
    this.state = {
      number1 : '',
  ...
  nextScreen = () => {
    ...
    formData.bankId      = this.state.id;
    formData.accountNo   = this.state.acNumber;
    formData.amount      = this.state.amount;
    formData.pin         = number1+number2+number3+number4;

    if(this.props.ProfileState.userBalance.balance > formData.amount){
      this.props.WithdrawAmount(formData);
    }else {
  ...
  componentWillReceiveProps(nextProps){
    if(nextProps.MoneyState.withdrawSuccess == true){
      this.props.navigation.navigate('WithdrawSuccessScreen', {amount : this.state.amount})
    }
  }
        ...
        <View style={styles.section12}>
            ...
            <View style={{paddingBottom: 15, marginHorizontal: 50,}}>
              ...
              <View>
                ...
                <TouchableOpacity 
                // onPress={()=>this.props.navigation.navigate('WithdrawSuccessScreen')}
...
function mapDispatchToProps(dispatch){
  return bindActionCreators({
    WithdrawAmount : AC_WithdrawAmount,
  }, dispatch)
}

export default connect(mapStateToProps, mapDispatchToProps)(WithdrawPinScreen);
