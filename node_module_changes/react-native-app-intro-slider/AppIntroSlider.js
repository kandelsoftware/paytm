...
export default class AppIntroSlider extends React.Component {
  ...
  _renderItem = (item) => {
    ...
    const bottomSpacer = (this.props.bottomButton ? (this.props.showSkipButton ? 22 : 0) + 22 : 0) + (isIphoneX ? 22: 0) + 50;
  ...
  _renderDefaultButton = (name) => {
    ...
    if (this.props.bottomButton) {
      content = <View style={[styles.bottomButton, (name === 'Skip' || name === 'Prev') && { backgroundColor: 'transparent', color: 'red' }, this.props.buttonStyle]}>{content}</View>
    }
    return content;
  }

  _renderOuterButton = (content, name, onPress) => {
    const style = (name === 'Skip' || name === 'Prev') ? styles.color : styles.color;
    return (
